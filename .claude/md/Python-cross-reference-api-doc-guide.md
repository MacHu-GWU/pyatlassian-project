...
### pyatlassian/auth/permissions.py
...
### pyatlassian/database/models.py
...
### pyatlassian/database/connection.py
...
def process_user():
    ...
    Creates a :class:`~pyatlassian.auth.users.User` instance and validates
    permissions through :class:`~pyatlassian.auth.permissions.Role`.
    """
    pass
```
...
def update_profile():
    ...
    Uses :meth:`User.get_profile <pyatlassian.auth.users.User.get_profile>` 
...
def login_user():
    ...
    Calls :func:`~pyatlassian.auth.users.authenticate` to verify credentials
    and :func:`~pyatlassian.auth.permissions.check_permission` for authorization.
    """
    pass
```
...
def setup_auth():
    ...
    Configures components from :mod:`pyatlassian.auth.users` and 
    :mod:`pyatlassian.auth.permissions` modules.
    """
    pass
```

**Rendered as**: Configures components from `pyatlassian.auth.users` and `pyatlassian.auth.permissions` modules.

### 2. Referencing Elements Within the Same Module

Use short paths when referencing elements in the same Python file:

#### Example: pyatlassian/auth/users.py
...
:ref:`getting-started` guide, then review the :class:`~pyatlassian.database.models.BaseModel`
class documentation.

For connection management, see :func:`~pyatlassian.database.connection.connect`
...
:func:`~pyatlassian.database.connection.connect`.
...
class BaseModel:
    ...
    through :func:`~pyatlassian.database.connection.connect`.
    
    Integration with authentication is handled through 
    :class:`~pyatlassian.auth.users.User` instances. User permissions are
    validated using :func:`~pyatlassian.auth.permissions.check_permission`.
    ...
    def save(self) -> None:
        ...
        Uses the connection established by :func:`~pyatlassian.database.connection.connect`.
        Requires authentication through :func:`~pyatlassian.auth.users.authenticate`.
        """
        pass
    ...
    def find_by_id(self, record_id: int) -> Optional[dict]:
        ...
        .. seealso::
            ...
            :func:`~pyatlassian.database.connection.Database.query` for custom queries.
        """
        pass
```
...
class UserManager:
    ...
    **Core Components:**
    ...
    - User creation: :class:`~pyatlassian.auth.users.User`
    - Authentication: :func:`~pyatlassian.auth.users.authenticate`
    - Authorization: :func:`~pyatlassian.auth.permissions.check_permission`
    - Data persistence: :class:`~pyatlassian.database.models.BaseModel`
...
:class:`pyatlassian.auth.users.User`
